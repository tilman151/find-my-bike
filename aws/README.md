...
```bash
aws$ sam local invoke InferenceFunction --event events/event.json
```
...
aws$ curl http://localhost:3000/classify_digit
```
...
```bash
aws$ sam logs -n InferenceFunction --stack-name aws --tail
```
